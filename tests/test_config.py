...
class TestLoadModelAliases:
    ...
    def test_aliases_normalized_to_lowercase(self, tmp_path):
        path = tmp_path / "model-aliases.json"
        path.write_text(json.dumps({"aliases": {"OpenAI/GPT-4O": "GPT-4O-MINI"}}))

        aliases = load_model_aliases(path)

        assert aliases == {"openai/gpt-4o": "gpt-4o-mini"}
...
class TestLoadLiteLLMConfig:
    ...
    def test_default_model_prices_url_derived_from_base_url(self, tmp_path):
        path = tmp_path / "litellm.json"
        path.write_text(json.dumps({"base_url": "https://litellm.internal"}))

        cfg = load_litellm_config(path)

        assert cfg.model_prices_url == "https://litellm.internal/model_prices_and_context_window.json"
