...
def compare_sessions_head_to_head(a: dict, b: dict) -> dict:
    """Compare two sessions and return a 'Winner' report with deltas.

    a, b: session objects as structured in _build_dashboard_json (models.py/dashboard.py)
    """
    def _safe_val(v): return v if v is not None else 0

    results = {}

    # 1. Quality Winner (Higher is better)
    qa = _safe_val(a.get("quality_score"))
    qb = _safe_val(b.get("quality_score"))
    results["quality"] = {
        "a": qa, "b": qb,
        "delta": qb - qa,
        "winner": "b" if qb > qa else ("a" if qa > qb else None)
    }

    # 2. Token Efficiency (Lower is better) - Tokens per event
    ev_a = max(1, _safe_val(a.get("event_count")))
    ev_b = max(1, _safe_val(b.get("event_count")))
    tok_a = _safe_val(a.get("input_tokens")) + _safe_val(a.get("output_tokens"))
    tok_b = _safe_val(b.get("input_tokens")) + _safe_val(b.get("output_tokens"))
    tpe_a = tok_a / ev_a
    tpe_b = tok_b / ev_b

    results["efficiency"] = {
        "a": round(tpe_a, 1), "b": round(tpe_b, 1),
        "delta_pct": round(((tpe_b - tpe_a) / tpe_a * 100), 1) if tpe_a > 0 else 0,
        "winner": "b" if tpe_b < tpe_a else ("a" if tpe_a < tpe_b else None)
    }

    # 3. Reliability (Lower failure rate is better)
    fail_a = _safe_val(a.get("failure_count"))
    fail_b = _safe_val(b.get("failure_count"))
    fr_a = fail_a / ev_a
    fr_b = fail_b / ev_b

    results["reliability"] = {
        "a": round(fr_a * 100, 1), "b": round(fr_b * 100, 1),
        "winner": "b" if fr_b < fr_a else ("a" if fr_a < fr_b else None)
    }

    # 4. Latency (Lower p50 is better)
    # Average of p50s across all tools present in both
    tools_a = a.get("tool_p50", {})
    tools_b = b.get("tool_p50", {})
    shared_tools = set(tools_a.keys()) & set(tools_b.keys())
    if shared_tools:
        avg_p50_a = sum(tools_a[t] for t in shared_tools) / len(shared_tools)
        avg_p50_b = sum(tools_b[t] for t in shared_tools) / len(shared_tools)
        results["latency"] = {
            "a": round(avg_p50_a, 1), "b": round(avg_p50_b, 1),
            "delta_pct": round(((avg_p50_b - avg_p50_a) / avg_p50_a * 100), 1) if avg_p50_a > 0 else 0,
            "winner": "b" if avg_p50_b < avg_p50_a else ("a" if avg_p50_a < avg_p50_b else None)
        }
    else:
        # Fallback to absolute duration if no shared tools
        dur_a = _safe_val(a.get("duration_ms"))
        dur_b = _safe_val(b.get("duration_ms"))
        results["latency"] = {
            "a": dur_a, "b": dur_b,
            "winner": "b" if dur_b < dur_a else ("a" if dur_a < dur_b else None),
            "note": "absolute duration (no shared tools)"
        }

    # 5. Summary Verdict
    winners = [v["winner"] for v in results.values() if v.get("winner")]
    win_a = winners.count("a")
    win_b = winners.count("b")
    results["verdict"] = "a" if win_a > win_b else ("b" if win_b > win_a else "tie")
